...
namespace FaisalLeague.Queries
{
    public class LoginQueryProcessor : ILoginQueryProcessor
    {
        ...
        public UserWithToken Authenticate(string username, string password)
        {
            ...
            var expiresIn = GlobalSettings.CURRENT_DATETIME + TokenAuthOption.ExpiresSpan;
            var token = _tokenBuilder.Build(user.Username,user.Id.ToString(), user.Roles.Select(x => x.Role.Name).ToArray(), expiresIn);
